...
function Home() {
  ...
  return (
    <div className="container wide flex">
      ...
      <div className="nav col-3 px-6 pb-12">
        <Link className="inline-block mt-4 mb-60 text-color" to="/admin/users">Vào trang admin</Link>
        ...
        <div className="favorites border-b-2 py-4">
          ...
          {publicStore.certificates?.sort((a, b) => Number(a.year) - Number(b.year)).map((certificate) => (
            <div className="py-1">
      ...
      <div className="content col-0 py-72 px-6">
        <div className="">
          ...
          <ul className="list-disc pl-4">
            {publicStore.schools?.sort((a, b) => Date.parse(a.start_time) - Date.parse(b.start_time)).map((school) => (
              <li className="py-2">
                <BlockItem title={school.name} start_date={changeISOTimeToMyFormTime(school.start_time)} end_date={changeISOTimeToMyFormTime(school.end_time)} >
        ...
        <div className="py-8">
          ...
          <ul className="list-disc pl-4">
            {publicStore.projects?.sort((a, b) => Date.parse(a.start_time) - Date.parse(b.start_time)).map((project) => (
              <li className="py-2">
                <BlockItem title={project.name} start_date={changeISOTimeToMyFormTime(project.start_time)} end_date={new Date(project.end_time) ? changeISOTimeToMyFormTime(project.end_time) : "đến nay"} >
